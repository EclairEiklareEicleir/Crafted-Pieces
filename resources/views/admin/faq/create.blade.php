...
<div class="rounded-4xl border border-brand-border bg-white p-6 shadow-sm">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-semibold text-brand-primary">Create New FAQ</h2>
            <p class="mt-2 text-sm text-brand-ink/70">Add a new frequently asked question and answer to the public page.</p>
        </div>
        <x-back-button href="{{ route('admin.faq.index') }}" label="Back to FAQ" />
    </div>

    <form method="POST" action="{{ route('admin.faq.store') }}" class="mt-8 space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="answer" rows="10" required
                      class="brand-input w-full" placeholder="Provide a detailed answer...">{{ old('answer') }}</textarea>
            @error('answer')
                <p class="mt-2 text-sm text-brand-secondary">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="flex items-end">
                <label class="flex items-center gap-2 text-sm text-brand-ink/70 cursor-pointer">
                    <input type="checkbox" name="active" value="1" {{ old('active', true) ? 'checked' : '' }} class="h-4 w-4">
        ...
        <div class="flex gap-3 pt-4">
            <button type="submit" class="brand-btn-primary px-6 py-3">Create FAQ</button>
