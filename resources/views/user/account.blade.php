...
<section class="mx-auto max-w-4xl px-4 py-14 sm:px-6 lg:px-8">
    <div class="rounded-[2rem] border border-[#eadfd7] bg-white p-6 shadow-sm sm:p-8">
        ...
        <p class="mt-3 text-sm text-[#6f5a51]">
            Update your customer account details used for orders and communication.
        </p>

        @if (session('status'))
            <p class="mt-6 rounded-2xl border border-[#e7d6cb] bg-[#fcfaf8] px-4 py-3 text-sm text-[#5d342b]">
                {{ session('status') }}
            </p>
        @endif
        ...
        <form method="POST" action="{{ route('account.update') }}" class="mt-8 space-y-5">
            ...
            <div>
                <label for="name" class="mb-2 block text-sm font-semibold text-[#5d342b]">
                    Full name
                </label>
            ...
            <div>
                <label for="email" class="mb-2 block text-sm font-semibold text-[#5d342b]">
                    Email address
                </label>
